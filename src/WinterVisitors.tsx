...
function Twig({ from, to }: { from: Vec; to: Vec }) {
  ...
  return <mesh position={a.add(b).multiplyScalar(0.5)} quaternion={rotation} >
    <cylinderGeometry args={[0.018, 0.027, direction.length(), 8]} />
    <meshStandardMaterial color="#c9ad83" />
  </mesh>;
}
function Reindeer({ tool, world }: { tool: Tool; world: World }) {
  const treat = useTreat(tool, world.time >= 6 && world.time < 19, "Reindeer", 1.5);
  ...
  useFrame((_, dt) => {
    ...
    if (treat.left.current > 0) {
      if (head.current) head.current.rotation.x = 0.12 + Math.sin(treat.left.current * 5) * 0.025;
      return;
    }
  ...
  return <group ref={root} position={[1.9, 0.27, 3.85]} rotation={[0, 1, 0]} scale={visible} name="winter-reindeer" onClick={treat.onClick}>
    {treat.feedback}
  ...
  return <group name="winter-visitors">
    ...
    <group position={[-2.5, 0.27, 3.3]} name="christmas-tree">
      ...
      <group position={[0, 1.79, 0]}>
        {[0, 1, 2, 3, 4].map(i => <mesh key={i} rotation={[0, 0, i * Math.PI * 2 / 5]} >
      ...
      {[[-0.55, 0.15, 0.45], [0.42, 0.12, 0.5]].map((p, i) => <group key={i} position={p as Vec}>
        <mesh castShadow ><boxGeometry args={[0.3, 0.24, 0.28]} /><meshStandardMaterial color={i ? "#91a99a" : "#b86762"} /></mesh>
        <mesh ><boxGeometry args={[0.055, 0.25, 0.29]} /><meshStandardMaterial color="#eddbb2" /></mesh>
        <mesh ><boxGeometry args={[0.31, 0.25, 0.045]} /><meshStandardMaterial color="#eddbb2" /></mesh>
      </group>)}
    </group>
  </group>;
}
